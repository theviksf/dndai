...
import { Terminal, Brain, Clock, Image, Copy, Book, Sparkles, Globe, FileText } from 'lucide-react';
...
export default function DebugLogViewer({ debugLog, isOpen, onClose }: DebugLogViewerProps) {
  ...
  const backstoryParserLogs = debugLog.filter(log => log.type === 'backstoryparser');
      ...
      <DialogContent className="max-w-6xl max-h-[80vh] flex flex-col">
        ...
        <Tabs defaultValue="all" className="flex-1 flex flex-col min-h-0">
          <TabsList className="grid w-full grid-cols-8">
            ...
            <TabsTrigger value="backstoryparser">BS Parser ({backstoryParserLogs.length})</TabsTrigger>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-sm">
                            {log.type === 'primary' ? 'Primary DM' : log.type === 'parser' ? 'Parser' : log.type === 'image' ? 'Image Generation' : log.type === 'backstory' ? 'Backstory' : log.type === 'backstoryparser' ? 'Backstory Parser' : log.type === 'revelations' ? 'Revelations' : log.type === 'lore' ? 'World Lore' : 'Unknown'}
                          </span>
          ...
          <TabsContent value="backstoryparser" className="flex-1 min-h-0">
            <ScrollArea className="h-[500px] w-full rounded-md border p-4">
              <div className="space-y-4">
                {backstoryParserLogs.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    No backstory parser logs yet.
                  </div>
                ) : (
                  backstoryParserLogs.map((log) => (
                    <div key={log.id} className="border rounded-lg p-4 space-y-3 bg-orange-500/5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <FileText className="w-4 h-4 text-orange-500" />
                          <span className="text-xs text-muted-foreground font-mono">{log.model}</span>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <div className="text-xs font-semibold text-foreground">Entity Updates Response:</div>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-6 px-2"
          ...
          <TabsContent value="revelations" className="flex-1 min-h-0">
            <ScrollArea className="h-[500px] w-full rounded-md border p-4">
              <div className="space-y-4">
                {revelationsLogs.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">
                    No revelations logs yet.
                  </div>
                ) : (
                  revelationsLogs.map((log) => (
                    <div key={log.id} className="border rounded-lg p-4 space-y-3 bg-yellow-500/5">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Sparkles className="w-4 h-4 text-yellow-500" />
                          <span className="text-xs text-muted-foreground font-mono">{log.model}</span>
