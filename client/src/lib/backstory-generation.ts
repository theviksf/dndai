...
// Parse a single entity's backstory for attribute updates
export async function parseSingleBackstoryForEntityUpdates(
  ...
  const timestamp = Date.now();
  const id = `backstoryparser-single-${timestamp}-${nanoid(6)}`;
  
  // Load the single-entity backstory parser prompt
  const promptResponse = await fetch('/prompts/backstoryparser-single.md');
  const systemPrompt = await promptResponse.text();
  const model = config.backstoryParserLLM;
  
  try {
    // Build context with ONLY this entity's data
    const context = buildSingleEntityContext(entityType, entity, gameState);
    
    const response = await apiRequest('POST', '/api/parse-single-backstory', {
      systemPrompt,
      context,
      entityType,
      entity,
    ...
    if (!response.ok) {
      ...
      return { 
        ...
        updates: {},
        summary: 'Error parsing backstory',
        debugLogEntry,
      };
    }
    ...
    const debugLogEntry: DebugLogEntry = {
      ...
      type: 'backstoryparser',
      prompt: data.fullPrompt || systemPrompt,
      response: data.rawResponse || JSON.stringify(data, null, 2),
      model: data.model || model,
      tokens: data.usage,
    };
    ...
    return {
      ...
      entityId: data.entityId || entity.id || 'unknown',
      updates: data.updates || {},
      summary: data.summary || 'No updates extracted',
      debugLogEntry,
    };
  } catch (error: any) {
    console.error('Failed to parse single backstory for entity updates:', error);
...
function buildSingleEntityContext(
  entityType: 'companion' | 'npc' | 'quest' | 'location',
  entity: any,
  gameState: GameStateData
): string {
  ...
  sections.push('# ENTITY TO PARSE');
  sections.push('');
  sections.push(`Entity Type: ${entityType}`);
  sections.push(`Entity ID: ${entity.id || 'unknown'}`);
  sections.push('');
  
  // Add entity data
  sections.push('## Current Entity Data');
  sections.push(JSON.stringify(entity, null, 2));
  sections.push('');
  
  // Add minimal game context for reference
  sections.push('## Game Context (for reference)');
  sections.push(`Current Location: ${gameState.location.name}`);
  sections.push(`Player Character: ${gameState.character.name} (${gameState.character.race} ${gameState.character.class})`);
  sections.push('');
