...
**Execution Flow**: Player action → Primary LLM streams narrative → Parser LLM updates game state → Revelations Agent tracks reveals → Image Agent generates visuals → Backstory Agent creates context → Lore Agent enriches world → Cost tracking updates.
