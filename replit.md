...
- **Backstory Parser Agent**: Analyzes generated backstories and extracts detailed entity updates (appearance, personality, relationships, critical memories) to automatically update NPC, companion, and location fields. Runs after all backstories for a round are generated, ensuring consistency and reducing manual data entry. Uses dedicated prompt in `backstoryparser.md` with customizable LLM selection. **Defensive Validation**: All parser updates are validated and normalized before application - string fields are converted to strings, objectives arrays are ensured to be arrays (not single objects), numeric fields are converted to numbers, preventing type mismatches that could crash the UI.
