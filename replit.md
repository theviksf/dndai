...
- **EntityDetailSheet**: Modern sidebar/sheet that opens when clicking on entity cards. Features wider layout (sm:max-w-3xl), full-width images with hover scaling (for entities with images), and prominent NPC status badges. Status indicators use green badge with Heart icon for alive NPCs and red badge with Skull icon for dead NPCs. All entities support in-place editing with comprehensive field updates. Supports NPCs, companions, quests, locations, and businesses. Quest detail panel displays quest title (with icon), type badge (Main/Side), description, objectives with checkboxes, progress tracking, and backstory when available.
