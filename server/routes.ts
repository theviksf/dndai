...
// POST /api/check-entity-consistency - Check entity consistency with backstory
router.post('/check-entity-consistency', async (req: Request, res: Response) => {
  ...
  try {
    const { systemPrompt, entity, entityType, backstory, model } = req.body;
    const key = getApiKey(req);
...
// 7. POST /api/chat/memories - Generate memories for NPCs and companions
router.post('/chat/memories', async (req: Request, res: Response) => {
  try {
    const { narrative, existingCompanions, existingNPCs, model } = req.body;
    const key = getApiKey(req);
    ...
    if ((!existingCompanions || existingCompanions.length === 0) && 
      ...
      return res.json({ memories: {}, usage: null, model });
    }
    
    const systemPrompt = `You are a Memories Agent for a D&D adventure game. Your job is to extract personal memories that NPCs and companions would form from the current turn's narrative.
...
- Use exact character names as they appear in the provided lists`;
    ...
    const userPrompt = `# This Turn's Narrative:
${narrative}

# Existing Companions (can receive memories):
${existingCompanions?.length > 0 ? existingCompanions.join(', ') : 'None'}

# Existing NPCs (can receive memories):
${existingNPCs?.length > 0 ? existingNPCs.join(', ') : 'None'}

Analyze the narrative and extract memories for any characters who had meaningful interactions. Return ONLY raw JSON.`;
    
    console.log('[MEMORIES] Generating memories using model', model);
    console.log('[MEMORIES] Existing companions:', existingCompanions);
    console.log('[MEMORIES] Existing NPCs:', existingNPCs);
    ...
    console.log('[MEMORIES] Raw response:', rawContent);
    ...
    console.log('[MEMORIES] Parsed memories:', memories);
    ...
    res.status(500).json({ 
      error: error.message,
      fullPrompt: 'Prompt not available',
      rawResponse: JSON.stringify({ error: error.message, stack: error.stack }, null, 2)
    });
  }
});
