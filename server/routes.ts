...
// Helper function to build memories context
...
// 6b. POST /api/chat/memories - Track character memories
router.post('/chat/memories', async (req: Request, res: Response) => {
  try {
    const { systemPrompt, narrative, gameState, model, newCompanionIds = [], newNPCIds = [] } = req.body;
    const key = getApiKey(req);
